...
export default function Settings() {
    ...
    const providers = ["perplexity", "openai", "anthropic", "cohere", "groq", "ollama"];

    const [log, setLog] = useState("")
    const [provider, setProvider] = useState("perplexity");
    const [token, setToken] = useState("");
    const [models, setModels] = useState(perplexityModelList);
    const [model, setModel] = useState("sonar-medium-online");
    ...
    const [perplexityStatus, setPerplexityStatus] = useState(true);
    const [openaiStatus, setOpenaiStatus] = useState(true);
    const [anthropicStatus, setAnthropicStatus] = useState(true);
    ...
    const saveLocal = () => {
        ...
        localStorage.setItem("perplexity-status", perplexityStatus.toString());
    ...
    const handleCheckChange = (e) => {
        ...
        if (prov === "perplexity") {
            setPerplexityStatus(e.target.checked); // Use "perplexity" consistently
        } else if (prov === "openai") {
    ...
    const configureOpenai = () => {
        ...
        setSystemPrompt(localStorage.getItem("openai-system-prompt") || "You are a helpful assistant. Please respond directly; niceties and pro forma intros/conclusions are not necessary. Focus on substance and accuracy. Take your time; the user would always prefer rigour over speed.");
    ...
    const configureAnthropic = () => {
        ...
        setSystemPrompt(localStorage.getItem("anthropic-system-prompt") || "You are a helpful assistant. Please respond directly; niceties and pro forma intros/conclusions are not necessary. Focus on substance and accuracy. Take your time; priorise rigour over speed.");
                                ...
                                <div key={index} className="w-full mt-1 gap-1 flex justify-center ">
                                    ...
                                    <div className="h-7 w-[50%] flex items-center">
                                        <div className="flex items-center">
                                            <input
                                                type="checkbox"
                                                defaultChecked={["perplexity", "anthropic", "openai"].includes(prov)}
                            ...
                            <div className="w-full mt-1 gap-1 flex justify-center ">
                                ...
                                <div className="h-7 w-[50%] flex items-center bg-gray-400/10 border border-gray-900/20 rounded">
                                    <input
                                        ...
                                        value={systemPrompt}
                                        defaultValue={"You are a helpful assistant. Please respond directly; niceties and pro forma intros/conclusions are not necessary. Focus on substance and accuracy. Take your time; the user would always prefer rigour over speed."}
